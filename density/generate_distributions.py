from util import densityQuery,fringeScore,structuralEntorpy
import pickle
import numpy as np
...
import argparse
import warnings
from mydata.generate_dataset import generate_smile
...
def generate_datasets(x,label):
    ...
    x, density, fringe, SE = my_generate_problem(x)
    ...
    print(np.array(density).reshape(-1,n_nodes,1).shape)
    feat = {"node_feat": x, # n_samples x n_nodes x 2
            ...
            "density_feat":np.array(density).reshape(-1,n_nodes,1) # n_samples x n_nodes x 1
            }
