...
import numpy as np
from util3 import fringeScore,structuralEntorpy
import scipy
...
#TMPDIR = Path('/dev/shm' if os.path.exists('/dev/shm') else os.environ.get('TMPDIR',
        #os.environ.get('TEMP',            
         #   tempfile.gettempdir())))
...
def get_hgs_executable(url="https://github.com/vidalt/HGS-CVRP.git"):
    ...
    if os.name == "nt":
        executable=os.path.join(cwd_build, "Release","hgs.exe")
    else: 
