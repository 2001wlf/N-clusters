...
def loss_node(y_pred_nodes, y_nodes):
    # y_pred_nodes = y_pred_nodes.permute(0, 2, 1) # batch_size x 1 x n_node * 1
    loss_nodes = nn.L1Loss(reduction="none")(y_pred_nodes, y_nodes)
    # loss_nodes = nn.NLLLoss(reduction="none")(y_pred_nodes, y_nodes)
    return loss_nodes
...
class SparseGCNModel(nn.Module):
    ...
    def forward(self, x_nodes, x_edges, edge_index, inverse_edge_index, y_nodes, edge_cw, n_edges):
        ...
        loss = loss_node(y_pred_nodes.view(batch_size,num_nodes),y_nodes)
        loss = loss.view(batch_size, num_nodes, 1)[loss_mask]
