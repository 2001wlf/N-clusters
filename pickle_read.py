...
def draw_dataset(data,id):
    ...
    #plt.show()
    plt.savefig(f"radius/mydata2/picture/{args.dist}/" + str(data.shape[0]) + f"{id}.png")
def read_data(n_node,k):
    mydata = pickle.load(open("radius/mydata2/val/" + str(n_node) + ".pkl", "rb"))
    ...
    for i in range(k):
        ...
        draw_dataset(now_data,id=i)
    return node_feat, edge_feat, edge_index, inverse_edge_index, density_feat
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Read and visualize dataset')
