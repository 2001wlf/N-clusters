...
def make_spiral(n_points=1000, noise=0.2):
    n = np.sqrt(np.random.rand(n_points)) * 2*np.pi  # 半径→角度
    x1 = np.cos(n) * n + np.random.randn(n_points) * noise
    y1 = np.sin(n) * n + np.random.randn(n_points) * noise
    x2 = -np.cos(n) * n + np.random.randn(n_points) * noise
    y2 = -np.sin(n) * n + np.random.randn(n_points) * noise

    X = np.vstack([np.column_stack([x1,y1]), np.column_stack([x2,y2])])
    y = np.hstack([np.zeros(n_points,dtype=int), np.ones(n_points,dtype=int)])
    return X, y
