...
def generate_i(gen_args):
    ...
    return p, q, f, s
def generate_datasets(n_instances=500,batch_idx=0,dist="uniform",n_clusters=10,partition="train"):
    parser = argparse.ArgumentParser()
    
    
    parser.add_argument('--n_nodes', type=int,default=100)
    n_neighbours = 20 #可能需要修改！！！！！
    
    
    parser.add_argument('--save_dir', type=Path)
    parser.add_argument('--partition', type=str, choices=['train', 'val', 'test'],default=partition)
    ...
    parser.add_argument('--dist', type=str, choices=['uniform', 'gm', 'subg', 'exp', 'student'], default=dist)
    ...
    save_path = "{}/{}/{}_{}_part_{}.pkl".format(args.save_dir,args.partition,args.n_nodes,args.dist,batch_idx)
    ...
    print(np.array(q).reshape(-1,n_nodes,1).shape)
    feat = {"node_feat": x, # n_samples x n_nodes x 2
            ...
            "density_feat":np.array(q).reshape(-1,n_nodes,1) # n_samples x n_nodes x 1
            }
...
if __name__ == "__main__":
    ...
    if partition=='train':
        ...
        TOTAL_INSTANCES = 5000      # 总共想要 5000 个
    BATCH_SIZE = 5000           # 每次生成 5000 个 (防止炸内存)
    N_BATCHES = TOTAL_INSTANCES // BATCH_SIZE
    dist='uniform'
    for batch_idx in range(N_BATCHES):
        print(f"\n=== Generating Batch {batch_idx + 1}/{N_BATCHES} ===")
        n_clusters = random.randint(2, 10)
        generate_datasets(BATCH_SIZE,batch_idx,dist,n_clusters,partition)
    dist='gm'
    for batch_idx in range(N_BATCHES):
        print(f"\n=== Generating Batch {batch_idx + 1}/{N_BATCHES} ===")
        n_clusters = 10
