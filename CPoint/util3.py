...
from coding_tree import bulit_knn_adjm, trans_to_tree, update_node, all_entory
...
def get_hgs_executable(url="https://github.com/vidalt/HGS-CVRP.git"):
    ...
    if not os.path.isdir(filedir):
        ...
        check_call("/home/vuser/anaconda3/bin/cmake .. -DCMAKE_BUILD_TYPE=Release", cwd=cwd_build, shell=True)
...
def densityQuery(xys):
    max_radius = np.sqrt((np.max(xys)-np.min(xys))**2*2)/2
    seps = [max_radius/4, max_radius/8, max_radius/16]
    p = np.zeros(len(xys))
    for sep in seps:
        neighbors_model = NearestNeighbors(
            radius=sep, algorithm='auto',
            leaf_size=30, metric='euclidean',
            metric_params=None, p=None, n_jobs=None)
        neighbors_model.fit(xys)
        # This has worst case O(n^2) memory complexity
        neighborhoods = neighbors_model.radius_neighbors(xys,
...
def projection_overlap(data, centers):
    label, distance = pairwise_distances_argmin_min(data, centers)
    center_dist = pairwise_distances(centers,centers)
    fringe_core = np.zeros(data.shape[0])
    for i in range(len(data)):
        for j in range(len(centers)):
            if label[i] == j:
                continue
            # 靠近交界处的程度
            fringe_core[i] = max(fringe_core[i], (np.dot(centers[label[i]] - data[i], centers[label[i]] - centers[j]) / center_dist[label[i]][j]) / (center_dist[j][label[i]] / 2))
    return fringe_core

def fringeScore(xys):
    fringe = np.zeros(xys.shape[0])
    k_range = [3, 5, 10]
    for k in k_range:
        kmeans = KMeans(init='k-means++', n_clusters=k).fit(xys)
        fringe += projection_overlap(xys, kmeans.cluster_centers_)
    return fringe / len(k_range)

def structuralEntorpy(xys):
    ne = 5
    gap = 0.2
    tree_depth = 3
    adj_mat = bulit_knn_adjm(xys, ne, gap * np.std(xys))
...
def dataset_read():
    ...
    for filepath in glob.glob(os.path.join(data_dir, '*.txt')):
        ...
        y = arr[:, -1].astype(int)
        yield X, y, filename
def kmeans_penalty_feature(X, centers, labels,
                        q_core=0.3,
                        alpha=10.0,
                        beta=5.0):
    """
    返回一列特征 (N,1)，让“簇核心点”和“非核心点”有很大数值差异。

    X      : (N, d)，已归一化到 [0,1] 也没关系
    centers: (K, d)
    labels : (N,)，每个点的簇标号
    q_core : 每个簇内部的“核心半径”分位数，例如 0.3
    alpha  : 控制靠近中心的 sigmoid 陡峭程度
    beta   : 控制 margin 的 sigmoid 陡峭程度
    """
    ...
    # 到所有中心的距离 (N,K)
    d_all = np.linalg.norm(X[:, None, :] - centers[None, :, :], axis=-1)

    # 最近、第二近中心的距离
    d_sorted = np.partition(d_all, 1, axis=1)  # 只保证前两小是有序的
    d1 = d_sorted[:, 0]  # 最近
    d2 = d_sorted[:, 1]  # 第二近

    # 1) 每个簇的“核心半径” r_j（q_core 分位数）
    core_r = np.zeros_like(d1)
    gap_scale = np.zeros_like(d1)
    eps = 1e-8
    ...
    for j in range(K):
        ...
        # 核心半径 = 簇内距离的 q_core 分位数
        r_j = np.quantile(d1_j, q_core)
        core_r[idx] = r_j + eps

        # 用 margin 的中位数做一个尺度，防止数值太小
        margin_j = d2[idx] - d1[idx]
        margin_pos = margin_j[margin_j > 0]
        if margin_pos.size == 0:
            s_j = np.median(d1_j)  # 退化情况，随便给个尺度
        else:
            s_j = np.median(margin_pos)
        gap_scale[idx] = s_j + eps

    # 2) 距离核心半径的相对位置：rel < 1 表示在核心内
    rel = d1 / core_r
    # 越小越好：rel < 1 -> f_close 接近 1；rel > 1 -> 急剧下降
    f_close = 1.0 / (1.0 + np.exp(alpha * (rel - 1.0)))

    # 3) 与第二近中心的 margin（越大越好）
    margin = np.maximum(0.0, d2 - d1)
    m_norm = margin / gap_scale
    f_margin = 1.0 / (1.0 + np.exp(-beta * m_norm))

    # 4) 组合两个信息
    score = f_close * f_margin

    # 5) 全局归一化到 [0,1]，便于跟其他特征拼
    s_min, s_max = score.min(), score.max()
    score = (score - s_min) / (s_max - s_min + 1e-8)

    return score.reshape(-1, 1)  # (N,1)