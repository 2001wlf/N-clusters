...
def SummaryOutliers(X, k, z, alpha, beta):
    """
...
def SummaryOutliersByMe(X, k, z, alpha, beta):
    """
                :param X: array of shape=(n_samples, n_features)
                :return
                """
    ...
    while len(X_i) > max(8 * z, 0):
        kappa = max(np.log(len(X_i)), k)
        S_i_size = int(alpha * kappa)
        # 6. construct a set S_i of size \alpha\kappa by random sampling (with replacement) from X_i
        S_i = np.random.choice(X_i, size=S_i_size, replace=True)
        S_i = np.unique(S_i)
        w_i = np.zeros((len(S_i),), dtype=np.int32)
        # 6. for each point in X_i, compute the distance to its nearest point in S_i
        # each value in nearest would range from 0 to len(S_i)
        stime = time.time()
        nearest, distance = pairwise_distances_argmin_min(X[X_i], X[S_i])
        # dis_map = spatial.distance.cdist(X[S_i], X[X_i])
        # dis_map = spatial.distance.cdist(X[S_i], X[X_i], 'euclidean')
        # nearest = np.argmin(dis_map, axis=0)
        # distance = np.min(dis_map, axis=0)
        etime = time.time()
        ...
        weights_.append(w_i)
        sample_indices_.append(S_i)
        assignment[X_i[distance <= rho_i]] += (len(S_i) + offset)
        offset += len(S_i)
...
if __name__ == "__main__":
    ...
    data, label = make_blobs(n_samples=n_samples, cluster_std=[1.5, 0.5, 1.5], random_state=random_state,
                             center_box=[-5.0, 5.0])
    # s, _ = make_blobs(n_samples=10, cluster_std=[1.5, 0.5, 1.5], random_state=random_state,
    #                          center_box=[-5.0, 5.0])
    # stime = time.time()
    # nearest, distance = pairwise_distances_argmin_min(data, s)
    # dis_map = spatial.distance.cdist(data, s, 'euclidean')
    # nearest = np.argmin(dis_map, axis=0)
    # distance = np.min(dis_map, axis=0)
    # etime = time.time()
    # print(etime - stime)
    ...
    #
    sstime = time.time()
    sample_indices_, assignment, weights_ = SummaryOutliers(data, k, 25, 1.0, 0.4)
    # setime = time.time()
    # print(setime - sstime)
    print(sample_indices_.shape)
    # #
    # sstime = time.time()
    # sample_indices_, assignment, weights_ = SummaryOutliersByMe(data, k, 25, 1.0,0.4)
    # setime = time.time()
    # print(setime-sstime)
    #
        ...
        for n in range(len(ind[1:])):
            if n:
                if dist[m][n + 1] > threshold:
                    continue
            j = ind[n + 1]
            adj_mat[i][j] = dist[m][n + 1] * weights_[i]
    # getime = time.time()
    # # print(getime - gstime)
    #
    # t1 = time.time()
