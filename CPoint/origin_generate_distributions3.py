from util3 import densityQuery,fringeScore,structuralEntorpy,multiprocess,kmeans_penalty_feature
import pickle
import numpy as np
...
def generate_problem(args, init=None):
    ...
    #Radius=raidusQuery(xys,kmeans.labels_)
    Cpoints=kmeans_penalty_feature(xys,kmeans.cluster_centers_, kmeans.labels_)  # (N,1)
    return xys, density, fringe, SE,Cpoints
...
def generate_i(gen_args):
    ...
    args.id = i
    p, q, f, s,c = generate_problem(args, init)
...
def generate_datasets():
    ...
    x,q,frige,s,Cpoints = zip(*results)
    x = np.array(x)
    ...
    print(np.array(Cpoints).reshape(-1,n_nodes,1).shape)
    feat = {"node_feat": x, # n_samples x n_nodes x 2
            ...
            "density_feat":np.array(Cpoints).reshape(-1,n_nodes,1) # n_samples x n_nodes x 1
            }
